...
(defn get-input-data
  []
  (let [input (slurp "resources/day06.part2.input")
...
(defn reallocate
  ...
  (loop [cycles 0
    ...
    (if (contains? configs curr-config)
      (let [originating-cycle (get configs curr-config)
            diff-cycles (- cycles originating-cycle)]
        diff-cycles)
      (let [next-config (redistribute curr-config max-index)]
        (recur (inc cycles)
               (assoc configs curr-config cycles)
