...
(defn partition-graph
  [graph]
  (loop [vertices (apply conj PersistentQueue/EMPTY (keys graph))
         vertex (peek vertices)
         visited #{}
         groups []]
    (cond
      (nil? vertex) groups
      (contains? visited vertex) (recur (pop vertices)
                                        (peek vertices)
                                        visited
                                        groups)
      :else (let [group (traverse graph vertex)]
              (recur (pop vertices)
                     (peek vertices)
                     (into visited group)
                     (conj groups group))))))
...
(defn run
  []
  (count (partition-graph input-graph)))
